...
const ResultsCard = ({ results }) => {
  ...
  const answeredQuestionsCount = results.filter(result => result.userAnswer !== 'Not Answered').length;
  const correctAnswersCount = results.filter(result => result.userAnswer === result.correctAnswer).length;
  const incorrectAnswersCount = answeredQuestionsCount - correctAnswersCount;
  ...
  return (
    <div className='mt-10 w-3/4 p-4 border rounded shadow-md'>
      ...
      <div className='mt-4'>
        ...
        <p className='text-lg mb-2'><span className='font-semibold'>Answered Questions:</span> {answeredQuestionsCount}</p>
